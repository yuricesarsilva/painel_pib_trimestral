...
\begin{frame}{Proxies setoriais: estrutura e pesos}
\begin{columns}[T]
\column{0.46\textwidth}
\textbf{Blocos principais}
\vspace{0.1cm}

\small
\begin{tabular}{p{2.1cm}rp{1.6cm}}
\toprule
\textbf{Bloco} & \textbf{Peso} & \textbf{Proxy} \\
\midrule
Adm.\ Pública & 45,01\% & folha pública \\
Serviços & 36,46\% & subblocos \\
Indústria & 11,63\% & subblocos \\
Agropecuária & 6,89\% & PAM/PPM \\
\bottomrule
\end{tabular}

\column{0.50\textwidth}
\textbf{Proxies compostas (pesos otimizados)}
\vspace{0.1cm}

\small
\begin{tabular}{p{2.0cm}p{3.6cm}}
\toprule
\textbf{Subbloco} & \textbf{Composição} \\
\midrule
Comércio & PMC 70\% + energia 10\% + ICMS 10\% + CAGED G 10\% \\
Transportes & pax ANAC 55\% + diesel 45\% \\
Financeiro & depósitos 60\% + crédito 40\% \\
Transformação & energia 55\% + CAGED C 45\% \\
\bottomrule
\end{tabular}
\end{columns}

\vspace{0.3cm}
\begin{block}{Como os pesos são definidos}
Pesos contábeis (entre blocos): participação no VAB de 2020 — Contas Regionais. Pesos técnicos (dentro dos subblocos): otimização por critério de variância do Denton (slide seguinte).
\end{block}
\end{frame}

\begin{frame}{Otimização dos pesos técnicos}
\begin{columns}[T]
\column{0.50\textwidth}
\textbf{Critério adotado}

\vspace{0.1cm}
\small
Proxy perfeita $\Rightarrow$ razão $p_t/x_t$ constante $\Rightarrow$ Denton não precisa corrigir. Pesos ótimos minimizam:
\[
\sum_{t=2}^{T} \!\left(\frac{p_t}{x_t} - \frac{p_{t-1}}{x_{t-1}}\right)^{\!2}
\]
Busca em grade, passo 5 p.p., 504 combinações, benchmark 2020--2023.
...
\vspace{0.1cm}
\small
\begin{tabular}{p{1.9cm}rr}
\toprule
\textbf{Setor} & \textbf{Ant.} & \textbf{Red.} \\
\midrule
Transformação & 0,0021 & $-$59,9\% \\
Transportes & 0,0350 & $-$41,7\% \\
Financeiro & 0,0025 & $-$90,5\% \\
Comércio & 0,0342 & conserv. \\
\bottomrule
\end{tabular}

\vspace{0.15cm}
\scriptsize Comércio: ótimo bruto seria 95\% PMC, mas piso de 10\% foi aplicado a todos os componentes. Adotado: PMC 70\% / energia 10\% / ICMS 10\% / CAGED G 10\%.
\end{columns}
\end{frame}
...
\addplot[fill=seplanblue] coordinates {
  (2020,16.02) (2021,18.20) (2022,21.10) (2023,25.12) (2024,28.83) (2025,32.25)
};
\end{axis}
\end{tikzpicture}
\end{columns}
...
\addplot[fill=seplangold] coordinates {
  (2021,8.40) (2022,11.30) (2023,4.20) (2024,6.43) (2025,6.55)
};
\end{axis}
\end{tikzpicture}
\end{columns}
...
\midrule
Adm. Pública & 15,71 & 47,8 \\
Serviços & 8,80 & 26,8 \\
Indústria & 4,94 & 15,0 \\
Agropecuária & 3,39 & 10,3 \\
\bottomrule
\end{tabular}
...
]{47.8/Adm.\ Pública,26.8/Serviços,15.0/Indústria,10.3/Agropecuária}
\end{tikzpicture}
\end{columns}
\end{frame}
