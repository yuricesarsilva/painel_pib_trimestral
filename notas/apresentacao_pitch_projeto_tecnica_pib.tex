...
\begin{frame}{Proxies setoriais: estrutura e fontes}
\small
\begin{tabular}{p{4.0cm}rp{5.0cm}}
\toprule
\textbf{Atividade} & \textbf{Peso 2020} & \textbf{Proxy principal} \\
\midrule
Adm.\ Pública & 45,01\% & Folha observada (SIAPE + estadual + municipal) \\
Serviços privados & 36,46\% & subblocos: comércio, transp., financeiro, outros \\
Indústria & 11,63\% & construção (CAGED F), SIUP (energia), transformação \\
Agropecuária & 6,89\% & PAM/LSPA + PPM/abate \\
\bottomrule
\end{tabular}

\vspace{0.2cm}
\textbf{Subblocos com proxies compostas (pesos otimizados):}

\begin{tabular}{p{3.4cm}p{6.8cm}}
\toprule
\textbf{Subbloco} & \textbf{Composição} \\
\midrule
Comércio & energia ANEEL (60\%) + ICMS SEFAZ-RR (20\%) + CAGED G (20\%) \\
Transportes & passageiros ANAC (55\%) + diesel ANP (45\%) \\
Financeiro & depósitos Estban (60\%) + concessões BCB (40\%) \\
Ind.\ Transformação & energia ANEEL (55\%) + emprego CAGED C (45\%) \\
\bottomrule
\end{tabular}
\end{frame}

\begin{frame}{Otimização dos pesos técnicos das proxies compostas}
\textbf{Critério: variância residual do Denton-Cholette}

\vspace{0.1cm}
Quando a proxy é perfeita, a razão $p_t / x_t$ é constante e o Denton não precisa corrigir nada. A função-objetivo do método Denton mede exatamente esse ruído:
\[
\text{Objetivo} = \sum_{t=2}^{T} \left(\frac{p_t}{x_t} - \frac{p_{t-1}}{x_{t-1}}\right)^2
\]

Os \textbf{pesos ótimos} minimizam esse objetivo sobre o período de benchmark 2020--2023. Busca em grade com passo de 5 p.p.\ (504 combinações avaliadas, script \texttt{05b\_sensibilidade\_pesos.R}).

\vspace{0.2cm}
\small
\begin{tabular}{lrrr}
\toprule
\textbf{Setor} & \textbf{Obj.\ anterior} & \textbf{Obj.\ ótimo} & \textbf{Redução} \\
\midrule
Ind.\ Transformação & 0,002118 & 0,000849 & 59,9\% \\
Comércio & 0,034204 & \emph{conservador} & --- \\
Transportes & 0,035024 & 0,020417 & 41,7\% \\
Financeiro & 0,002532 & 0,000240 & 90,5\% \\
\bottomrule
\end{tabular}

\vspace{0.15cm}
\scriptsize Comércio: ótimo seria 100\% energia, mas conservamos diversificação (histórico do ICMS por atividade limitado a 2020--2023). Decisão documentada em \texttt{notas/otimizacao\_pesos\_proxies.md}.
\end{frame}
...
\addplot[fill=seplanblue] coordinates {
  (2020,16.02) (2021,18.20) (2022,21.10) (2023,25.12) (2024,28.96) (2025,32.58)
};
\end{axis}
\end{tikzpicture}
\end{columns}
...
\addplot[fill=seplangold] coordinates {
  (2021,8.40) (2022,11.30) (2023,4.20) (2024,6.91) (2025,7.16)
};
\end{axis}
\end{tikzpicture}
\end{columns}
...
\begin{itemize}
  ...
  \item Os pesos técnicos das proxies compostas foram otimizados com apenas quatro anos de benchmark (2020--2023); serão re-estimados após a publicação das CR 2024 (previsão: outubro de 2026).
