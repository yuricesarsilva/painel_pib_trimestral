...
O script `R/05h_vab_nominal_setorial.R` complementa essa frente com a série trimestral do
`VAB nominal` dos quatro blocos do projeto (`Agropecuária`, `AAPP`, `Indústria` e `Serviços`)
nos anos com benchmark das Contas Regionais (`2020–2023`). O procedimento usa benchmark nominal
anual por bloco, índice anual de volume agregado com pesos de 2020, deflator anual por bloco e
trimestralização via Denton-Cholette com IPCA como proxy do perfil intra-anual de preços.
...
| Arquivo | Conteúdo |
|---|---|
| `IAET_RR_series.xlsx` | Excel com 7 abas: Índice Geral, Componentes, Dessazonalizado, Fatores Sazonais, Metadados, VAB Nominal, PIB Nominal |
