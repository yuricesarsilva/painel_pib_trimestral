...
| 🟡 Média | Obter dados mensais do SNIC (cimento) — script já aceita o CSV e ativa CAGED F 60% + SNIC 40% automaticamente | SNIC |
...
| SEFAZ-RR | ICMS estadual mensal — proxy do ILP trimestral | Download manual; `data/processed/icms_sefaz_rr_mensal.csv` |
| SNIC | Cimento vendido em RR — componente opcional da Construção | Download manual; `data/raw/snic_cimento_rr.csv` (ativa CAGED F 60% + SNIC 40%) |
